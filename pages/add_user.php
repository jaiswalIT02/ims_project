...
if(isset($_POST['submit'])){
    ...
    $role = $_POST['role'];
    $sql = mysqli_query($conn, "SELECT * FROM `login_master` WHERE `user_login` = '$username'");
    $count = mysqli_num_rows($sql);
    if($count > 0) {
        
        $_SESSION['str'] = 'Error';
        $_SESSION['msg'] = "Already this username pls choose another one ";
       // header('location:add_user.php');
    } else {
        
        $result = mysqli_query($conn, "INSERT INTO `login_master` (`co_id`,`user_name`,`user_login`,`user_password`, `desig_id`) VALUES ('','$name','$username','$password','$role', '1');" );
        if($result){
...
{        
    ...
    if($query)
    {
        for($i=0;$i<sizeof($outlets);$i++)
        {
           ...
           echo $sql_ins = "INSERT INTO `user_outlet_access`(`co_id`, `LoginName`, `UserLevel`, `UnitId`, `UnitName`) VALUES ('$_SESSION[co_id]' , '$login_name' , '$desig_id' , '$ol_id' , '$ol_name')";
